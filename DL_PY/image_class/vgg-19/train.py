...
model = vgg("vgg19", 256, 256, 3)
model.summary()

# using keras high level api for training
model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001), #Adam梯度下降算法
              loss=tf.keras.losses.CategoricalCrossentropy(from_logits=False), #CategoricalCrossentropy  交叉熵损失函数
              metrics=["accuracy"])
