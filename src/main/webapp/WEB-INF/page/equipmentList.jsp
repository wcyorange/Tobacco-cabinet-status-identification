...
<script>
    layui.config({
        ...
        table.render({
            ...
            ,url: 'equipment/listjson' //修改这个地址，列表查询路径，数据类型见json/table/demo.js
            ,toolbar: '#test-table-toolbar-toolbarDemo'
            ,title: '数据表'
            ,cols: [[
                {field:'id', title:'id', align:'center',sort:true,fixed: 'left'}
                ,{field:'code', title:'设备编号', align:'center',width: 120}
                ,{field:'ip', title:'设备IP地址', align:'center',width: 120}
                ,{field:'username', title:'设备登录用户名', align:'center',width: 120}
                ,{field:'password', title:'设备登录密码', align:'center',width: 120}
                ,{field:'port', title:'设备端口号', align:'center',width: 120}
                ,{field:'liveUrl', title:'直播地址', align:'center',width: 120}
                ,{field:'name', title:'柜号', align:'center',width: 120}
                ,{field:'cate', title:'摄像头所在位置', align:'center',width: 120}
                ,{field:'status', title:'摄像头状态', align:'center',width: 120,templet:function(d){
                        if(d.status ==1)
                            return '<span style="color: #00801e">1-运行中</span>';
                        else
                            return '<span style="color: red">2-停止运行</span>';
                    }}
                ,{field:'liveImg', title:'直播图片', align:'center',width: 120,templet: '<div><img id="imgs" style="width: 100%; height:100%;border-radius: 50% " onerror="onImgError(this);" onclick="previewImg(this)" src="layuiadmin/touxiang/{{d.liveImg}}"></div>'}
                ,{field:'statusMsg', title:'状态说明', align:'center',width: 120}
        ...
        table.on('toolbar(test-table-toolbar)', function(obj){
            if(obj.event=="add"){
                parent.layui.index.openTabsPage("equipment/add", "添加参数");
            }
        });
        ...
        table.on('tool(test-table-toolbar)', function(obj){
            ...
            if(obj.event === 'del'){
                ...
                layer.confirm('真的删除行么', function(index){
                    admin.req({
                        type: "POST"
                        ,url: 'equipment/delete' //实际使用请改成服务端真实接口
                        ,data: data
                        ,done: function(res){
                            if(res){
                                obj.del();
                ...
                parent.layui.index.openTabsPage("equipment/editForm?id="+obj.data.id, "修改参数");
            }
