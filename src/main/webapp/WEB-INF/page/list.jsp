...
layui.config({
	...
	table.render({
		...
		,title: '用户数据表'
		,cols: [[
			 {filed:'uId',title:'序列', type:'numbers', width:80, fixed: 'left' }
			,{field:'uLoginName', title:'登陆名', fixed: 'left', width:100}
			,{field:'uName', title:'用户名', fixed: 'left', width:100}
			,{field:'email', title:'邮箱', width:150}
			,{field:'mobile', title:'手机号', width:100}
			,{field:'lastLoginIp', title:'登陆Ip'}
			,{field:'lastLoginTime', title:'登陆时间', width:120}
			,{field:'flatformAdminGroup', title:'角色', width:100,templet:function (data) {
					return data.flatformAdminGroup.agName;
				}}
    ...
    table.on('tool(test-table-toolbar)', function(obj){
		...
		if(obj.event === 'del'){
			...
			parent.layui.index.openTabsPage("user/echodata?uId="+obj.data.uId, "修改用户");
		}
    }); 
});
</script>
