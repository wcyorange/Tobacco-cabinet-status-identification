...
<script>
    layui.config({
        ...
        table.render({
            ...
            ,url: 'cgMenu/listjson' //修改这个地址，列表查询路径，数据类型见json/table/demo.js
            ,toolbar: '#test-table-toolbar-toolbarDemo'
            ,title: '数据表'
            ,cols: [[
                {field:'id', title:'id', align:'center',width: 120,sort:true,fixed: 'left'}
                ,{field:'userId', title:'创建分组用户Id', align:'center',width: 220}
                ,{field:'cgId', title:'自定义分组Id', align:'center',width: 220}
        ...
        table.on('toolbar(test-table-toolbar)', function(obj){
            if(obj.event=="add"){
                parent.layui.index.openTabsPage("cgMenu/addForm", "添加参数");
            }
        });
        ...
        table.on('tool(test-table-toolbar)', function(obj){
            ...
            if(obj.event === 'del'){
                ...
                layer.confirm('真的删除行么', function(index){
                    admin.req({
                        type: "POST"
                        ,url: 'cgMenu/delete' //实际使用请改成服务端真实接口
                        ,data: data
                        ,done: function(res){
                            if(res){
                                obj.del();
                ...
                parent.layui.index.openTabsPage("cgMenu/updateForm?id="+obj.data.id, "修改参数");
            }
