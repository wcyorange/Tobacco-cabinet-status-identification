...
public class Preview {
    ...
    private Logger logger = LoggerFactory.getLogger(Preview.class);
    ...
    public void getHkwsCamera(Camera camera, int passage) {
        ...
        File file = new File("E:\\HkvsDir");
        if (!file.exists()) {
            file.mkdir();
        }

        //预览成功后 调用接口使视频资源保存到文件中
        if(!hcNetSDK.NET_DVR_SaveRealData(key, file.getAbsolutePath()+ "\\result.mp4")){
